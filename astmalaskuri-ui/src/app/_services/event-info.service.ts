...
export class EventInfoService {
  ...
  getMedUseEvents(): Observable<IEventInfo[]> {
    return this.http.get<IEventInfo[]>(this.baseUrl + 'eventinfo/');
  }
   ...
   getEvent(id: number): Observable<IEventInfo> {
    return this.http.get(`${this.baseUrl}eventinfo/${id}`).pipe(
  ...
  useFlixotide(newInfo: IEventInfo): any {
    const response = this.http.post(`${this.baseUrl}eventinfo/1`, newInfo).subscribe((data) => {
      return data;
    });
    return response;
  }
  useVentoline(newInfo: IEventInfo): any {
    const response = this.http.post(`${this.baseUrl}eventinfo/2`, newInfo).subscribe((data) => {
      return data;
    });
    return response;
  }
